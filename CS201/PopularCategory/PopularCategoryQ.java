    ...
    public String popular(String words, String categories) {
        ...
        for (int i = 0; i < wordArray.length; i++) {
            ...
            if (!categoryToWords.containsKey(category)) {
                categoryToWords.put(category, new ArrayList<>());
            }
            categoryToWords.get(category).add(word);
        }
        String maxCategory = null;
        int maxCount = 0;
        for (Map.Entry<String, List<String>> entry : categoryToWords.entrySet()) {
        ...
        List<String> wordsInCategory = categoryToWords.get(maxCategory);
