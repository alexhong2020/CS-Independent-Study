...
def count(phrase):
    phrase = phrase.split()
    phrase = [len(word) for word in phrase]
    phrase = dict(Counter(phrase))
    max_value = max(phrase.values())
    max_key = [key for key, value in phrase.items() if value == max_value]
    max_key = sorted(max_key)
    if len(max_key) == 1:
        return str(max_value) + ':' + str(max_key[0])
    else:
        return str(max_value) + ':' + '-'.join(map(str, max_key))
...
def count(phrase):
    ...
    phrase = dict(Counter(phrase))
    max_value = max(phrase.values())
    max_key = [key for key, value in phrase.items() if value == max 
    '''    