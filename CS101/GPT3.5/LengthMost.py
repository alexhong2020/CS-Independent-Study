...
def count(phrase):
    ...
    # Iterate through the words and count the lengths
    for word in words:
        length = len(word)
    ...
    # Find the lengths that have the maximum count
    most_lengths = [length for length, count in word_length_counts.items() if count == max_count]

    # Sort the most_lengths list in numeric order
    most_lengths.sort()

    # Format the result string
    result = f"{max_count}:{'-'.join(map(str, most_lengths))}"
