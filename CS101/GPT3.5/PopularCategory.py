...
def popular(words, categories):
    ...
    # Iterate through the words and categories
    ...
    max_word_count = max(len(words) for words in category_counts.values())

    # Find all categories with the maximum word count
    max_categories = [category for category, words in category_counts.items() if len(words) == max_word_count]

    # Sort the max_categories list in alphabetical order
    max_categories.sort()

    # Get the words from the first category in max_categories
    most_popular_words = category_counts[max_categories[0]]

    # Sort the most_popular_words
    most_popular_words.sort()
