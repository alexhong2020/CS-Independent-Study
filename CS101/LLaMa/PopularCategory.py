...
def popular(words, categories):
    ...
    for i in range(len(words)):
        if words[i] not in word_dict:
            ...
            word_dict[words[i]].append(categories[i])
    word_list = []
    for key in word_dict:
        word_list.append((key, len(word_dict[key])))
    word_list.sort(key=lambda x: x[1], reverse=True) 
    if len(word_list) > 1 and word_list[0][1] == word_list[1][1]:
        word_list.sort(key=lambda x: x[0])
    return ' '.join([word[0] for word in word_list[0][1]])