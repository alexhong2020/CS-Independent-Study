...
def count(phrase):
    ...
    for word in words:
        if word not in word_lengths:
            ...
            word_lengths[word] += 1
    max_word_length = 0
    max_word_length_count = 0
    for word in word_lengths:
        if word_lengths[word] > max_word_length_count:
            max_word_length_count = word_lengths[word]
            max_word_length = word
    return str(max_word_length_count) + ":" + str(max_word_length)